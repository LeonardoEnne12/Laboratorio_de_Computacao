...
void assemblyGen_LOAD(Quad q){                                                                                                      // Tratamento da quadrupla LOAD
    if(isNumber(q.arg2)){                                                                                                           // Se o inteiro a se der load vier de uma constante
        ...
        if(st_lookup(q.arg2,"global") == 1){                                                                                        // Se o escopo da variavel for global
            ...
            else{                                                                                                                   // Se nao possuir um indice de acesso
                if(st_lookup_size(q.arg2,currentScope) > 1)                                                                         // Se for um vetor e necessitar passar o endereço como PARAM
                    assemblyInsert(ADDI, tempRegister(q.arg1), $zero, 0, location, lineNo, I, Inst, "");                            // Usa-se o ADDI para guardar o endereço base da variavel
                else
            ...
            if(strcmp(q.arg3,"-") != 0){                                                                                            // Se for um vetor e precisar acessar algum indice
                ...
                if(st_lookup_size(q.arg2,currentScope) > 1)                                                                         // Se for um vetor e necessitar passar o endereço como PARAM
                    assemblyInsert(ADDI, tempRegister(q.arg1), $sp, 0, location, lineNo, I, Inst, "");                              // Usa-se o ADDI para guardar o endereço base da variavel
                else
